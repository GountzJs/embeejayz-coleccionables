...
export function PageBLG() {
	const { stickers, isLoading } = useStickersContext();
	const blgStickers = json["blg"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const logo = blgStickers.find((sticker) => sticker.role === "LOGO");
	const coach = blgStickers.find((sticker) => sticker.role === "COACH");
	const team = blgStickers.find((sticker) => sticker.role === "TEAM");
		...
		>
			<div className="flex items-center justify-center flex-wrap gap-4 h-fit w-full">
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					...
					{!isLoading && logo && <StickerLogo team="BLG" />}
				</StickerCover>
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					{isLoading && <></>}
					{!isLoading && coach && (
...
export function PageBLGBack() {
	const { stickers, isLoading } = useStickersContext();
	const blgStickers = json["blg"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const proplayers = blgStickers.filter(
		(sticker) =>
			sticker.role !== "TEAM" &&
			sticker.role !== "LOGO" &&
			sticker.role !== "COACH",
