...
export function PageKOI() {
	const { stickers, isLoading } = useStickersContext();
	const koiStickers = json["koi"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const logo = koiStickers.find((sticker) => sticker.role === "LOGO");
	const coach = koiStickers.find((sticker) => sticker.role === "COACH");
	const team = koiStickers.find((sticker) => sticker.role === "TEAM");
		...
		>
			<div className="flex items-center justify-center flex-wrap gap-4 h-fit w-full">
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					...
					{!isLoading && logo && <StickerLogo team="KOI" />}
				</StickerCover>
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					{isLoading && <></>}
					{!isLoading && coach && (
...
export function PageKOIBack() {
	const { stickers, isLoading } = useStickersContext();
	const koiStickers = json["koi"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const proplayers = koiStickers.filter(
		(sticker) =>
			sticker.role !== "TEAM" &&
			sticker.role !== "LOGO" &&
			sticker.role !== "COACH",
