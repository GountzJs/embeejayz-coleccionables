...
export function PageAL() {
	const { stickers, isLoading } = useStickersContext();
	const alStickers = json["al"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const logo = alStickers.find((sticker) => sticker.role === "LOGO");
	const coach = alStickers.find((sticker) => sticker.role === "COACH");
	const team = alStickers.find((sticker) => sticker.role === "TEAM");
		...
		>
			<div className="flex items-center justify-center flex-wrap gap-4 h-fit w-full">
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					...
					{!isLoading && logo && <StickerLogo team="AL" />}
				</StickerCover>
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					{isLoading && <></>}
					{!isLoading && coach && (
...
export function PageALBack() {
	const { stickers, isLoading } = useStickersContext();
	const alStickers = json["al"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const proplayers = alStickers.filter(
		(sticker) =>
			sticker.role !== "TEAM" &&
			sticker.role !== "LOGO" &&
			sticker.role !== "COACH",
