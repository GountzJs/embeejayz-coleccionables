...
export function PageKT() {
	const { stickers, isLoading } = useStickersContext();
	const ktStickers = json["kt"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const logo = ktStickers.find((sticker) => sticker.role === "LOGO");
	const coach = ktStickers.find((sticker) => sticker.role === "COACH");
	const team = ktStickers.find((sticker) => sticker.role === "TEAM");
		...
		>
			<div className="flex items-center justify-center flex-wrap gap-4 h-fit w-full">
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					...
					{!isLoading && logo && <StickerLogo team="KT" />}
				</StickerCover>
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					{isLoading && <></>}
					{!isLoading && coach && (
...
export function PageKTBack() {
	const { stickers, isLoading } = useStickersContext();
	const ktStickers = json["kt"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const proplayers = ktStickers.filter(
		(sticker) =>
			sticker.role !== "TEAM" &&
			sticker.role !== "LOGO" &&
			sticker.role !== "COACH",
