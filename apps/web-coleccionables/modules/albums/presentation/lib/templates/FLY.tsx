...
export function PageFLY() {
	...
	const flyStickers = json["fly"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const logo = flyStickers.find((sticker) => sticker.role === "LOGO");
	const coach = flyStickers.find((sticker) => sticker.role === "COACH");
	const team = flyStickers.find((sticker) => sticker.role === "TEAM");
		...
		>
			<div className="flex items-center justify-center flex-wrap gap-4 h-fit w-full">
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					...
					{!isLoading && logo && <StickerLogo team="FLY" />}
				</StickerCover>
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					{isLoading && <></>}
					{!isLoading && coach && (
...
export function PageFLYBack() {
	const { stickers, isLoading } = useStickersContext();
	const flyStickers = json["fly"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const proplayers = flyStickers.filter(
		(sticker) =>
			sticker.role !== "TEAM" &&
			sticker.role !== "LOGO" &&
			sticker.role !== "COACH",
