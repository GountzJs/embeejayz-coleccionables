...
export function PageIG() {
	const { stickers, isLoading } = useStickersContext();
	const igStickers = json["ig"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const logo = igStickers.find((sticker) => sticker.role === "LOGO");
	const coach = igStickers.find((sticker) => sticker.role === "COACH");
	const team = igStickers.find((sticker) => sticker.role === "TEAM");
		...
		>
			<div className="flex items-center justify-center flex-wrap gap-4 h-fit w-full">
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					...
					{!isLoading && logo && <StickerLogo team="IG" />}
				</StickerCover>
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					{isLoading && <></>}
					{!isLoading && coach && (
...
export function PageIGBack() {
	const { stickers, isLoading } = useStickersContext();
	const igStickers = json["ig"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const proplayers = igStickers.filter(
		(sticker) =>
			sticker.role !== "TEAM" &&
			sticker.role !== "LOGO" &&
			sticker.role !== "COACH",
