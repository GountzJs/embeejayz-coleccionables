...
export function PagePSG() {
	const { stickers, isLoading } = useStickersContext();
	const psgStickers = json["psg"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const logo = psgStickers.find((sticker) => sticker.role === "LOGO");
	const coach = psgStickers.find((sticker) => sticker.role === "COACH");
	const team = psgStickers.find((sticker) => sticker.role === "TEAM");
		...
		>
			<div className="flex items-center justify-center flex-wrap gap-4 h-fit w-full">
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					...
					{!isLoading && logo && <StickerLogo team="PSG" />}
				</StickerCover>
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					{isLoading && <></>}
					{!isLoading && coach && (
...
export function PagePSGBack() {
	const { stickers, isLoading } = useStickersContext();
	const psgStickers = json["psg"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const proplayers = psgStickers.filter(
		(sticker) =>
			sticker.role !== "TEAM" &&
			sticker.role !== "LOGO" &&
			sticker.role !== "COACH",
