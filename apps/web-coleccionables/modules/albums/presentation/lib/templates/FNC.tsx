...
export function PageFNC() {
	const { stickers, isLoading } = useStickersContext();
	const fncStickers = json["fnc"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const logo = fncStickers.find((sticker) => sticker.role === "LOGO");
	const coach = fncStickers.find((sticker) => sticker.role === "COACH");
	const team = fncStickers.find((sticker) => sticker.role === "TEAM");
		...
		>
			<div className="flex items-center justify-center flex-wrap gap-4 h-fit w-full">
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					...
					{!isLoading && logo && <StickerLogo team="FNC" />}
				</StickerCover>
				<StickerCover width="40%" variant="VERTICAL" aspectRatio="200 / 280">
					{isLoading && <></>}
					{!isLoading && coach && (
...
export function PageFNCBack() {
	const { stickers, isLoading } = useStickersContext();
	const fncStickers = json["fnc"].filter((sticker) =>
		stickers.some((id) => sticker.id === id),
	);
	const proplayers = fncStickers.filter(
		(sticker) =>
			sticker.role !== "TEAM" &&
			sticker.role !== "LOGO" &&
			sticker.role !== "COACH",
